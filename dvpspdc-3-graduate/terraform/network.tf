...
resource "yandex_dns_recordset" "www_recordset" {
  ...
  name    = "www.sergey-belov.ru."
  type    = "A"
  ttl     = 600
  data    = [yandex_compute_instance.main.network_interface.0.nat_ip_address]
}

resource "yandex_dns_recordset" "gitlab_recordset" {
  zone_id = yandex_dns_zone.dns_zone.id
  name    = "gitlab.sergey-belov.ru."
  type    = "A"
  ttl     = 600
  data    = [yandex_compute_instance.main.network_interface.0.nat_ip_address]
}
...
resource "yandex_dns_recordset" "prometheus_recordset" {
  zone_id = yandex_dns_zone.dns_zone.id
  name    = "prometheus.sergey-belov.ru."
  type    = "A"
  ttl     = 600
  data    = [yandex_compute_instance.main.network_interface.0.nat_ip_address]
}
...
resource "yandex_dns_recordset" "alertmanager_recordset" {
  zone_id = yandex_dns_zone.dns_zone.id
  name    = "alertmanager.sergey-belov.ru."
  type    = "A"
  ttl     = 600
  data    = [yandex_compute_instance.main.network_interface.0.nat_ip_address]
}
